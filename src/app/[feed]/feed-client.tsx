...
type InitialCommunityFeed = { id: string; name: string; description: string; creator: string; followers: number };

interface FeedClientProps {
  // When the server-side data fetch in page.tsx resolves, it passes the
  // results here so we can render the first paint without a client
  // round-trip. `null` means "no SSR data, do the full client fetch on
  // mount" — covers any fallback path.
  initialSlug?: string;
  initialItems?: FeedItem[] | null;
  initialHasMore?: boolean;
  initialNextCursor?: string | null;
  initialCommunityFeed?: InitialCommunityFeed | null;
...
export default function FeedPage({
  ...
  initialCommunityFeed = null,
  initialNotFound = false,
}: FeedClientProps = {}) {
  const params = useParams();
  const feedSlug = (params.feed as string) || "tech";
  const activeTab = ALL_SYSTEM_FEEDS.find((t) => t.id === feedSlug);
  // SSR initial data only applies if the slug we're rendering matches the
  // slug the server fetched. After in-app navigation to a different feed,
  // we fall through to the regular SWR/fetch path.
  const hasInitial = Array.isArray(initialItems) && initialItems.length > 0 && initialSlug === feedSlug;

  const [items, setItems] = useState<FeedItem[]>(initialItems || []);
  const [newArticlesAvailable, setNewArticlesAvailable] = useState(0);
  const [communityFeed, setCommunityFeed] = useState<InitialCommunityFeed | null>(initialCommunityFeed);
  ...
  useEffect(() => {
    // Three sources of "already painted" content, in priority order:
    //   1. initialItems prop — the server pre-rendered this slug's articles.
    //   2. localStorage cache — same slug visited recently.
    //   3. Nothing — full skeleton, full fetch.
    // In any case we still kick off a background fetch to refresh staleness;
    // the user just doesn't have to see a skeleton while it runs.
    const cacheKey = `mf_cache_${feedSlug}`;
    let hadContent = false;

    if (hasInitial) {
      // page.tsx already seeded items/hasMore/cursor/communityFeed via props.
      // Persist them to localStorage so subsequent visits hit the SWR path
      // even if SSR can't render (e.g. on a 4xx slug).
      try {
        localStorage.setItem(cacheKey, JSON.stringify({
          items: initialItems,
          hasMore: !!initialHasMore,
          cursor: initialNextCursor,
          feed: initialCommunityFeed,
          ts: Date.now(),
        }));
      } catch {}
      hadContent = true;
          ...
          <div ref={mobileTabs.ref} className="flex overflow-x-auto scrollbar-hide items-center gap-0 h-11">
            ...
            {allTabs.map((tab) => (
              <Link
                ...
                onTouchStart={() => prefetchFeed(tab.id)}
                onFocus={() => prefetchFeed(tab.id)}
                className={`shrink-0 flex items-center gap-1.5 px-2.5 py-2.5 text-xs whitespace-nowrap border-b-2 transition-colors h-full ${feedSlug === tab.id ? "border-text text-text font-semibold" : "border-transparent text-text-muted hover:text-text font-medium"}`}
              >
        ...
        {loading ? (
          ...
          <div className="space-y-2 pt-1">
            {filteredItems.map((item, i) => {
              ...
              // Strip the " - Publisher" suffix we just pulled out into the chip.
