...
interface ForYouClientProps {
  // SSR initial payload — server fetched the default For You blend so the
  // first paint already has cards. Null/empty means "do the full client
  // fetch on mount" (used as a fallback for any upstream error).
...
export default function ForYouClient({
  ...
  useEffect(() => {
    // Stale-while-revalidate: render whatever we already have on screen (SSR
    // initial items, or last-cached localStorage payload) without a spinner
    // flash, then refresh from the network in the background.
    let cancelled = false;
    const cacheKey = `myfeed:for-you:v1:${feedNames.join(",")}`;
    const CACHE_MAX_AGE_MS = 30 * 60_000; // 30 min — bounded staleness
    // SSR-served items already on screen → treat as fresh-enough for first
    // paint, no spinner. The fetch below refreshes in the background.
    let hadCache = items.length > 0;
    if (!hadCache) {
      try {
